...
#     def to_vec3D(vec): return Vector.add_z(
#         vec, design.get_chip_z(options['chip']))

#     options = {**DEFAULT_OPTIONS['draw_cpw_trace'],
#                **options}  # update with custom options
...
#     cpw_origin = points[0]
#     # distance vector, unit dist. vector, normal unit vector
#     vec_D, vec_d, vec_n = Vector.difference_dnt([points[0], points[1]])
...
#     obj_polyline_center_gp, obj_polyline_center_gpN = make_obj_polyline_track(
#         name+'_cut', cpw_center_gap)
#     if options['do_mesh'] in TRUE_STR:
#         obj_polyline_center_ms, obj_polyline_center_msN = make_obj_polyline_track(
#             'mesh_'+name, cpw_mesh_gap)

#     # MAKE SHEETS
#     if not (options['do_only_lines'] in TRUE_STR):
#         sheet_center_trace = oModeler._sweep_along_path(
#             obj_polyline_center_trN, obj_polyline_center_tr)
#         sheet_gap_trace = oModeler._sweep_along_path(
#             obj_polyline_center_gpN, obj_polyline_center_gp)
#         if options['do_mesh'] in TRUE_STR:
#             sheet_mesh_trace = oModeler._sweep_along_path(
#                 obj_polyline_center_msN, obj_polyline_center_ms)
...
#         design.connectors[name+'_start'] = make_connector(
#             [cpw_origin-dist, cpw_origin+dist], options)

#         vec_D, vec_d, vec_n = Vector.difference_dnt([points[-2], points[-1]])
#         # TODO: might need to play with orientation here
#         dist, cpw_end = cpw_center_w/2.*vec_n, points[-1]
#         raise NotImplemented('Update make_connector -- add to design!?')
#         design.connectors[name +
#                           '_end'] = make_connector([cpw_end-dist, cpw_end+dist], options)

#     if 1:
#         design.mesh_obj(str(sheet_mesh_trace),
#                         options['mesh_name'], **options['mesh_kw'])
...
#     if orient_HFSS is -1:
#         if plot_control:
#             plt.plot(points0[:, 1], orient_HFSS*points0[:, 0],
#                      marker='o', ms=3, alpha=0.6, label=label)
#         plt.plot(points[:, 1], orient_HFSS*points[:, 0],
#                  marker='o', ms=3, alpha=0.6)
#         ax.set_ylabel('-X')
#         ax.set_xlabel('Y')
#     else:
#         if plot_control:
...
#         design.get_option_values(
#             options, 'spacing, width, shift_fraction, lead_in_spacing')
...
#     # distance vector, unit dist. vector, normal unit vector
#     vec_D, vec_d, vec_n = Vector.difference_dnt(points0)
...
#     Nmax = int(np.floor((norm(np.dot(vec_D, vec_d)) -
#                          lead_in_spacing) / (meander_spacing)))
#     Steps = [[+ds*meander_out*s1*vec_n, meander_spacing*vec_d, -ds*meander_out*s2*vec_n],
#              [-ds*meander_out*s1*vec_n, meander_spacing *
#               vec_d, +ds*meander_out*s2*vec_n]
#              ]
...
#         #dist  = norm(pts)
#         # span enoguh space
#         line = LineString([points[-1]+unitv*100, points[-1]-unitv*100])
...
#         design, points[start_idx: start_idx+2], meander_options)
#     points = np.delete(points, [start_idx, start_idx+1], axis=0)
#     points = np.insert(points, start_idx, meander, axis=0)
#     # it is possible to get 3 colinear points
#     points = remove_colinear_pts(points)
#     return points
...
#                **options}  # overwritten for Metal Object
...
#         else:
#             raise Exception(
#                 'Object {obj} does not have obj.components.cpw_line')
...
#             else:
#                 raise Exception(
#                     'Unkown object {obj} does not have obj.components.cpw_line')
#         else:
#             raise Exception(
#                 'Unkown object {obj}: does not have obj.components')
#     elif isinstance(obj, str):
#         points_meander = np.array(unparse_units(
#             design.track_objs[options['category']][obj]['points']))
#         # DEPRICATED here
...
#     start, stop, step = [options[n.strip()]
#                          for n in ('start, stop, step'.split(','))]
...
#         vec_D, vec_d, vec_n = Vector.difference_dnt([p1, p2])
#         # print(p1, vec_n, norm(vec_D), '\n ',p1,p2, vec_n ) #render([Point(p1),Point(p2)])

#         # if the segment is longer than thresohld place a bond
#         if (norm(vec_D) > th) and (norm(vec_D)/2. > ofst):
