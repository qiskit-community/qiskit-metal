...
def intersecting(a: np.array, b: np.array, c: np.array, d: np.array) -> bool:
    ...
    if (x0_start == x0_end) and (x1_start == x1_end):
        ...
        if x0_end == x1_start:
            ...
            return not (
                (min(y0_start, y0_end) > max(y1_start, y1_end))
                or (min(y1_start, y1_end) > max(y0_start, y0_end))
            )
        ...
        if (x1_end - x1_start) * (y0_end - y0_start) == (x0_end - x0_start) * (
            ...
            if b0 == b1:
                ...
                return not (
                    (min(x0_start, x0_end) > max(x1_start, x1_end))
                    or (min(x1_start, x1_end) > max(x0_start, x0_end))
                )
...
class RouteAnchors(QRoute):
    ...
    def unobstructed_close_up(self, segment: list, component_name: str) -> bool:
        ...
        for _, row in paths.iterrows():
            paths_converted.append(
                row["geometry"].buffer(row["width"] / 2, cap_style=CAP_STYLE.flat)
            )
    ...
    def connect_simple(self, start_pt: QRoutePoint, end_pt: QRoutePoint) -> np.ndarray:
        ...
        if (start[0] == end[0]) or (start[1] == end[1]):
            ...
            if mao.dot(start_direction, end - start) >= 0:
                ...
                if (end_direction is None) or (
                    mao.dot(end - start, end_direction) <= 0
                ):
            ...
            if (mao.dot(start_direction, corner1 - start) > 0) and startc1end:
                ...
                if (end_direction is None) or (
                    mao.dot(end_direction, corner1 - end) >= 0
                ):
                ...
                if (end_direction is None) or (
                    mao.dot(end_direction, corner2 - end) >= 0
                ):
            ...
            if stop_direction[
                0
            ]:  # "Wide" rectangle -> vertical middle segment is more natural
            ...
            if (
                (mao.dot(start_direction, stop_direction) < 0)
                and (mao.dot(start_direction, corner3 - start) > 0)
                and startc3c4end
            ):
                if (end_direction is None) or (
                    mao.dot(end_direction, corner4 - end) > 0
                ):
            ...
            if (mao.dot(start_direction, corner5 - start) >= 0) and startc5c6end:
                if (end_direction is None) or (
                    mao.dot(end_direction, corner6 - end) >= 0
                ):
    ...
    def free_manhattan_length_anchors(self):
        ...
        for i in range(1, len(reference)):
            length += abs(reference[i][0] - reference[i - 1][0]) + abs(
                reference[i][1] - reference[i - 1][1]
            )
        return length
    ...
    def make(self):
        ...
        self.trim_pts()
        self.intermediate_pts = np.concatenate(
            list(self.intermediate_pts.values()), axis=0
        )
