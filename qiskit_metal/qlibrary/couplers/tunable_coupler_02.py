...
class TunableCoupler02(BaseQubit):
    ...
    Default Options:
        ...
        * bus_inner_length: '0.5mm' -- the length of the CPW between the JJ and the circular pad 
        * bus_outer_length: '0.5mm' -- the length of the CPW between the circular pad and the in/out pin 
        * bus_JJ_height: '0.5mm' -- the vertical distance between the main cpw line and the offset JJ
    ...
    def make(self):
        ...
        JJ = draw.LineString([(-0.5*p.JJ_length, p.bus_JJ_height - 0.5*p.JJ_width),(0.5*p.JJ_length, p.bus_JJ_height - 0.5*p.JJ_width)])
        
        # draw the individual components of the CPW resonator 
        bus_vertical_left = draw.rectangle(p.bus_width, p.bus_JJ_height, -0.5*p.JJ_length - 0.5*p.bus_width, 0.5*p.bus_JJ_height)
        bus_vertical_right = draw.rectangle(p.bus_width, p.bus_JJ_height, 0.5*p.JJ_length + 0.5*p.bus_width, 0.5*p.bus_JJ_height)
        bus_left = draw.rectangle(p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius,
                                  p.bus_width, 
                                  -0.5*p.JJ_length - p.bus_width -0.5*(p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius),
                                  0.5*p.bus_width) 
        bus_right = draw.rectangle(p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius,
                                  p.bus_width, 
                                  0.5*p.JJ_length + p.bus_width + 0.5*(p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius),
                                  0.5*p.bus_width) 
        
        # draw the circular charge pads 
        left_pad = draw.Point(-0.5*p.JJ_length - p.bus_width - p.bus_inner_length - 0.5*p.pad_radius,0.5*p.bus_width).buffer(p.pad_radius)
        right_pad = draw.Point(0.5*p.JJ_length + p.bus_width + p.bus_inner_length + 0.5*p.pad_radius,0.5*p.bus_width).buffer(p.pad_radius)
        ...
        objects = [JJ, bus_vertical_left, bus_vertical_right, bus_left, bus_right, left_pad, right_pad, fbl, pocket]
        objects = draw.rotate(objects, p.orientation, origin=(0, 0))
        objects = draw.translate(objects, xoff=p.pos_x, yoff=p.pos_y)
        [JJ, bus_vertical_left, bus_vertical_right, bus_left, bus_right, left_pad, right_pad, fbl, pocket] = objects
        ...
        # Right-hand pin
        qp1b = (0.5*p.JJ_length + p.bus_width + p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius, 0.5*p.bus_width)
        qp1a = (0.5*p.JJ_length + p.bus_width + 0.9*p.bus_inner_length + p.bus_outer_length + 2.0*p.pad_radius, 0.5*p.bus_width)
        ...
        # Left-hand pin
        qp2b = (-0.5*p.JJ_length - p.bus_width - p.bus_inner_length - p.bus_outer_length - 2.0*p.pad_radius, 0.5*p.bus_width)
        qp2a = (-0.5*p.JJ_length - p.bus_width - 0.9*p.bus_inner_length - p.bus_outer_length - 2.0*p.pad_radius, 0.5*p.bus_width)
        ...
        # Flux Bias Line pin
        qp3b = (-0.5*p.JJ_length - p.bus_width - p.fbl_offset - 0.5*p.fbl_width, p.bus_JJ_height + p.fbl_length)
        qp3a = (-0.5*p.JJ_length - p.bus_width - p.fbl_offset - 0.5*p.fbl_width, p.bus_JJ_height + 0.9*p.fbl_length)
