...
class TransmonConcentricType2(BaseQubit):
    ...
    def make(self):
        ...
        cutout = draw.rectangle(8.0*p.finger_N_width, p.finger_N_length,
                                0.0, p.rad_inner + 0.5*p.finger_N_length)
        ...
        JJ = draw.LineString([(p.rad_inner*cos(45*3.14159/180),
        ...
        finger_NE = draw.LineString([(p.rad_outer*cos(45*3.14159/180),
        ...
        coupler_NE_cut = draw.rotate(coupler_NE_cut,
                                     -45, origin=(l*cos(45*3.14159/180),
                                                  l*sin(45*3.14159/180)))
        coupler_NE = draw.subtract(coupler_NE, coupler_NE_cut)
        # draw the northwest finger
        finger_NW = draw.LineString([(-1.0*p.rad_outer*cos(45*3.14159/180),
                                      p.rad_outer*sin(45*3.14159/180)),
        ...
        coupler_NW = draw.rotate(coupler_NW, 45,
        ...
        finger_SW = draw.LineString([(-1.0*p.rad_outer*cos(45*3.14159/180),
        ...
        coupler_SW = draw.rotate(coupler_SW, 135,
        ...
        finger_E = draw.LineString([(-1.0*p.rad_outer,0.0),
                                    (-1.0*(p.rad_outer + p.finger_E_length),0.0)])
        box = draw.rectangle(p.box_E_width, p.box_E_length,
                             -1.0*(p.rad_outer + p.finger_E_length + 0.5*p.box_E_width), 0.0)
        # draw the north finger with the triangular coupling pad
        finger_N = draw.LineString([(0.0, p.rad_inner), (0.0, p.rad_inner + p.finger_N_length)])
        # draw the top triangular coupling pad
        padtop_i = draw.rectangle(p.triangle_base, p.triangle_height,
                                  0.0,
        ...
        objects = [outer_pad, inner_pad, JJ, finger_NE,
                   ...
                   finger_N, padtop, coupler_NE, coupler_NW, coupler_SW]
        objects = draw.rotate(objects, p.orientation, origin=(0, 0))
        objects = draw.translate(objects, xoff=p.pos_x, yoff=p.pos_y)
        [outer_pad, inner_pad, JJ, finger_NE, finger_NW, finger_SW,
         finger_E, box, finger_N, padtop, coupler_NE, coupler_NW, coupler_SW] = objects
        ##############################################################
        ...
        self.add_qgeometry('junction', geom_jj, layer=1, subtract=False, width=p.jj_w)
        self.add_qgeometry('path', geom_finger_NE, layer=1, subtract=False, width=p.finger_NE_width)
        self.add_qgeometry('path', geom_finger_NW, layer=1, subtract=False, width=p.finger_NW_width)
        self.add_qgeometry('path', geom_finger_SW, layer=1, subtract=False, width=p.finger_SW_width)
        self.add_qgeometry('path', geom_finger_E, layer=1, subtract=False, width=p.finger_E_width)
        self.add_qgeometry('poly', geom_box_E, layer=1, subtract=False)
        self.add_qgeometry('path', geom_finger_N, layer=1, subtract=False, width=p.finger_N_width)
        ...
        qp1a = (qp1a_r*cos(45*3.14159/180), qp1a_r*sin(45*3.14159/180))
        qp1b_r = p.rad_outer + p.finger_NE_length + p.coupler_NE_gap + p.coupler_NE_width
        qp1b = (qp1b_r*cos(45*3.14159/180), qp1b_r*sin(45*3.14159/180))
        ...
        qp2a = (-1.0*qp2a_r*cos(45*3.14159/180), qp2a_r*sin(45*3.14159/180))
        qp2b_r = p.rad_outer + p.finger_NW_length + p.coupler_NW_gap + p.coupler_NW_width
        qp2b = (-1.0*qp2b_r*cos(45*3.14159/180), qp2b_r*sin(45*3.14159/180))
        ...
        qp3a = (-1.0*qp3a_r*cos(45*3.14159/180), -1.0*qp3a_r*sin(45*3.14159/180))
        qp3b_r = p.rad_outer + p.finger_SW_length + p.coupler_SW_gap + p.coupler_SW_width
        qp3b = (-1.0*qp3b_r*cos(45*3.14159/180), -1.0*qp3b_r*sin(45*3.14159/180))
