...
class QNet():
    ...
    def __init__(self):
        ...
        self._net_info.append(pd.Series([2, 3, 'qubit'],index=_net_info.columns), ignore_index=True)
        self._net_info.append(pd.Series([2, 3, 'qubit'],index=_net_info.columns), ignore_index=True)
        self._net_info.append(pd.Series([2, 3, 'qubit'],index=_net_info.columns), ignore_index=True)

        self._net_table.add((1, 'read1', 3, 'qubit' ))
        self._net_table.add((1, 'bus1', 4, 'qstart' ))
        self._net_table.add((2, 'bus2', 5, 'qend' ))
