...
class Vec3D:
    ...
    def dist(self, other):
        return np.round(
            np.sqrt((self.x - other.x)**2 + (self.y - other.y)**2 + (self.z - other.z)**2),
            decimals=9)

    def translate(self, other, ret_new_obj:bool=False):
        if not ret_new_obj:
            self.x += other.x
            self.y += other.y
            self.z += other.z
    ...
    def rotate(self, cvec, ax:bool=False, ay:bool=False, az:bool=False, angle:float=0.0, ret_new_obj:bool=False):
        x = self.x; y = self.y; z = self.z
        if ax:
            new_y = (y - cvec.y)*np.cos(angle) - (z - cvec.z)*np.sin(angle) + cvec.y
            new_z = (y - cvec.y)*np.sin(angle) + (z - cvec.z)*np.cos(angle) + cvec.z
            y = new_y; z = new_z

        if ay:
            new_z = (z - cvec.z)*np.cos(angle) - (x - cvec.x)*np.sin(angle) + cvec.z
            new_x = (z - cvec.z)*np.sin(angle) + (x - cvec.x)*np.cos(angle) + cvec.x
            x = new_x; z = new_z

        if az:
            new_x = (x - cvec.x)*np.cos(angle) - (y - cvec.y)*np.sin(angle) + cvec.x
            new_y = (x - cvec.x)*np.sin(angle) + (y - cvec.y)*np.cos(angle) + cvec.y
            y = new_y; x = new_x
...
class Vec3DArray:
    ...
    @staticmethod
    def make_vec3DArray(points:list[Union[int, float]], chip_z:float=None):
        vecs = []
        for xyz in points:
            if len(xyz) > 2:
                ...
                if chip_z is not None:
                    ...
                    raise TypeError(f"Expected a chip_z value for 2D geometry, found NoneType.")
            else:
                raise ValueError(f"Expected a 2-D or 3-D coordinate, got {len(xyz)}-D instead.")

        return Vec3DArray(vecs)

def vecs_to_gmsh_points(vecs:list[Vec3D], chip_z:float):
    points = []
    for v in vecs:
...
def line_width_offset_pts(pt_vec:Vec3D, path_vec:Vec3D, width:float, chip_z:float, ret_pts:bool = True):
...
def arc_width_offset_pts(vec1:Vec3D, vec3:Vec3D, angle:float, width:float, chip_z:float):
    # TODO: change this to use Vec3D translate and rotate
...
def transform_arc_points(pts:list, translate:Vec3D, path_vecs:list, chip_z:float):
...
def draw_curves(recent_pts:list, curves1:list, curves2:list):
    rec_pts = []
