...
from .widgets.library_new_qcomponent.parameter_entry_dockified import create_qcomponent_parameter_entry
if TYPE_CHECKING:
...
class MetalGUI(QMainWindowBaseHandler):
    ...
    def create_new_component_object_from_qlibrary(self,
        ...
        full_path = self.ui.dockLibrary.library_model.filePath(
            self.library_proxy_model.mapToSource(relative_index))
        try:
            self.qcpe = create_qcomponent_parameter_entry(
                self, self.QLIBRARY_FOLDERNAME, full_path, self.design)
            self.qcpe.show()
