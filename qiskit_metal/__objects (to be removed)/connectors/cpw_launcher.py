...
from ...renderers.renderer_ansys.draw_ansys import do_cut_ground, do_PerfE
...
DEFAULT_OPTIONS['cpw_launcher'] = Dict({
    ...
    # +1 or -1, '10um' position along edge
    'position': "['X', -1, '7mm']",
...
class cpw_launcher(Metal_Object):
    ...
    def make(self):
        ...
        # Inner piece
        line_inner = shapely.wkt.loads(
            f'LINESTRING (0 0, {Bw} 0, {Bw} {Bh}, {Cw} {Bh+Lh})')
        # Outside piece which gets cut from ground plane
        line_outter = LineString(
            array(line_inner.coords) + array(((0, -Bg), (+Bg, -Bg), (+Bg, 0), (Cg, 0))))
        #
        line_mesh = LineString(array(line_outter.coords) +
                               array(((0, -Mg), (+Mg, -Mg), (+Mg, 0), (Mg, 0))))

        # Turn into poygons
        poly_inner = Polygon(draw.flip_merge(line_inner))
        poly_outer = Polygon(draw.flip_merge(line_outter))
        poly_mesh = Polygon(draw.flip_merge(line_mesh))
        ...
        if len(pos) == 3:
            ...
            raise ValueError(
                "Cannot interpreit position of chip. Check how you specified it!! See the help of this class. ")

        # Move and orient
        poly_inner, poly_outer, poly_mesh = draw.rotate_position([
            poly_inner, poly_outer, poly_mesh],  angle, pos1)
        ...
        # Connectors
        start = 3
        points = draw.get_poly_pts(poly_inner)[start:start+2]
    ...
    def hfss_draw(self):
        ...
        def to_vec3D(vec): return draw.vec_add_z(
            vec, to_ansys_units(self.design.get_chip_z(options['chip'])))
        ...
        if options['do_draw']:  # Draw
            ...
            def render(poly, name_sup, flag=None):
                ops = self.design.parse_value(options['kw_poly'])
                if not flag is None:
                    ops = {**ops, **{'transparency': flag}}
                line = oModeler.draw_polyline(
                    to_vec3D(draw.get_poly_pts(poly)), closed=True, **ops)
                line = line.rename(name + name_sup)
                return line

            Poly0 = render(poly0, '', 0)
            Poly1 = render(poly1, '_cut')
            Poly2 = render(polyM, '_mesh', 1) if options['do_mesh'] else None
