...
class CapExtraction(QAnalysisRenderer):
    ...
    def capacitance_matrix(self) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: Capactiance matrix data, typically generated by run_sim().
        """
    ...
    def capacitance_matrix(self, data: pd.DataFrame):
        ...
        Args:
            data (pd.DataFrame): Capactiance matrix data, typically generated by run_sim().
        """
    ...
    def capacitance_all_passes(self) -> dict:
        ...
        Returns:
            dict: of pd.DataFrame. For each pass of the incremental mash refinments,
    ...
    def capacitance_all_passes(self, data: dict):
        ...
        Args:
            data (dict): of pd.DataFrame. For each pass of the incremental mash refinments,
                it retains the capacitance matrix at that pass.
        """
        if not isinstance(data, dict):
            self.logger.warning(
                'Unuspported type %s. Only accepts dicts of pandas dataframes. Please try again.',
                {type(data)})
            return
