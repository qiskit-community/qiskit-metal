...
class ThreeFingerCapV1(QComponent):
   ...
   #  TODO _img = 'ThreeFingerCapV1.png'
   ...
   #  Define structure functions    
    ...
    def make(self):
        ...
        # make the shapely polygons for the built in leads in the pocket (length=pocket_buffer_width_y)
        cpw_temp_1 = draw.rectangle(p.cpw_width, p.pocket_buffer_width_y)
        cpw_top = draw.translate(cpw_temp_1, 0,
            +(p.finger_length+p.cpw_width*3+p.pocket_buffer_width_y*2)/2-p.pocket_buffer_width_y/2)
        ...
        top_pin_line = draw.LineString([(-p.cpw_width/2,(p.finger_length+p.cpw_width*3+p.pocket_buffer_width_y*2)/2),
            (+p.cpw_width/2,(p.finger_length+p.cpw_width*3+p.pocket_buffer_width_y*2)/2)])
        bot_pin_line = draw.LineString([(-p.cpw_width/2,-(p.finger_length+p.cpw_width*3+p.pocket_buffer_width_y*2)/2),
            (+p.cpw_width/2,-(p.finger_length+p.cpw_width*3+p.pocket_buffer_width_y*2)/2)])
        ...
        self.add_qgeometry('poly', dict(pad_top=pad_top, pad_bot=pad_bot, cent_finger=cent_finger,
            left_finger=left_finger, right_finger=right_finger, cpw_top=cpw_top, cpw_bot=cpw_bot),
            layer=p.layer)
