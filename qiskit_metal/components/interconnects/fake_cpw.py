...
class FakeCPW(QComponent):
    ...
    def make(self):
        ...
        fake_cpw_line = draw.LineString([starting_pin_dic['middle'],ending_pin_dic['middle']])
        self.add_elements('path', {f'{self.name}_cpw_line':fake_cpw_line}, width=self.p.line_width)

        self.add_pin('fake_cpw_start', starting_pin_dic.points, self.id, flip=True)
