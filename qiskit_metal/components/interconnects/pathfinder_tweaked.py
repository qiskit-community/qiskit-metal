...
# TODO: anchors causes issues with parsing (?)
# TODO: no_obstacles method should exclude starting point, which always intersects bounding box of starting component
# TODO: Rounding errors - end has actually been reached but A* doesn't actually recognize it
# TODO: Stopping condition for A* in case it doesn't converge (time limit?)
...
#         anchors=OrderedDict() # Intermediate anchors only; doesn't include endpoints
    ...
    def getpts_astar(self, start_direction: np.array, start: np.array, end: np.array, step_size: float=0.5) -> list:
        ...
        pathmapper = {(0, starting_dist, start[0], start[1]): [start]} # maps tuple(remaining_dist, length_travelled, coordx, coordy) to path
        visited = set([(start[0], start[1])]) # maintain record of points we've already visited to avoid self-intersections
        h = [(0, starting_dist, start[0], start[1])] # priority queue (heap in Python implementation)
...
#         anchors = p.anchors
        ...
        for coord in [m2 + n2 * (w / 2 + leadend)]:
            ...
            if easypath:
                ...
                self._pts += self.getpts_astar(coord - self._pts[-1], self._pts[-1], coord)
