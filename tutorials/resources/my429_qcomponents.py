...
class MyQComponent1(QComponent):
    ...
    def make(self):
        ...
        self.add_qgeometry("poly", {"my_polygon": rect}, layer=1, subtract=False)
        self.add_pin("in", rect.exterior.coords[:-3:-1], 0.1)  # name, tangent, width
...
class MyQComponent2(QComponent):
    ...
    default_options = Dict(
        width="0.5mm", height="0.1mm", pos_x="0mm", pos_y="0mm", layer="1"
    )
    """Default drawing options"""

    def make(self):
        """calculates the geometries of the QComponent"""
        p = self.parse_options()  # short-handle alias for the options interpreter

        rect = draw.rectangle(p.width, p.height, p.pos_x, p.pos_y)
        self.add_qgeometry("poly", {"my_polygon": rect}, layer=p.layer, subtract=False)
...
class MyQComponent3(QComponent):
    ...
    )  # gds imports and analysis inputs
    """Component metadata"""
...
class MyQComponent4(QComponent):
    ...
    default_options = Dict(
        width="0.5mm", height="0.1mm", gap="0.02mm", pos_x="0mm", pos_y="0mm", layer="1"
    )
    """Default drawing options"""
    ...
    def make(self):
        ...
        self.add_qgeometry(
            "path", {"trace": line}, width=p.height, layer=p.layer, subtract=False
        )
        line2 = draw.LineString(
            [((-p.width / 2) - 2 * p.gap, 0), ((p.width / 2) + 2 * p.gap, 0)]
        )
