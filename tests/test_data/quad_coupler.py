...
class QuadCoupler(QComponent):
    ...
    def make(self):
        ...
        coupler_sub_pad = draw.rectangle(
            p.pad_width + p.pad_gap * 2, p.pad_height + p.pad_gap * 2
        )

        yoff = (p.pad_height + p.cpw_stub_height) / 2
        cpw_conn = draw.LineString(
            [[0, -p.pad_height / 2], [0, -p.pad_height / 2 - p.cpw_stub_height]]
        )
        ...
        self.add_qgeometry(
            "poly", {"quad_coupler_sub": coupler_sub_pad}, layer=p.layer, subtract=True
        )

        self.add_qgeometry(
            "path", {"stub": cpw_conn}, layer=p.layer, width=p.cpw_stub_width
        )
        self.add_qgeometry(
            "path",
            {"stub_sub": cpw_conn_sub},
            layer=p.layer,
        ...
        self.add_pin(
            "in", points=np.array(pin_list), width=p.cpw_stub_width, input_as_norm=True
        )
