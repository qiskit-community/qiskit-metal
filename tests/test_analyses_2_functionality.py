...
from qiskit_metal.analyses.hamiltonian.states_energies import (
    basis_state_on,
    extract_energies,
)
...
class TestAnalyses(unittest.TestCase, AssertionsMixin):
    ...
    def test_analyses_cpw_guided_wavelength(self):
        ...
        test_a_expected = (0.024345363624151843, 2.46280979514796, 0.49998185730956207)
        test_b_expected = (0.02900577391226639, 2.460840094957294, 0.4999524962937447)
        test_c_expected = (0.02135485699188455, 2.807698502630373, 0.49998185730956207)
        test_d_expected = (0.025443724808268103, 2.805350708925153, 0.4999524962937447)
    ...
    def test_analyses_cpw_lumped_cpw(self):
        ...
        expected = (
            0.04160856394637145,
            3.7752144497164426e-07,
            2.9513334836287544e-11,
            0.0,
            113.09975672923031,
            1.0000000003871512,
            2.9513334836287544e-11,
        ...
        # Generate actual result data
        result = cpw_calculations.lumped_cpw(
            1000000.75,
            0.25,
            0.1,
            0.002,
            ...
            loss_tangent=0.1,
            london_penetration_depth=1 * 10**0.7,
        )
    ...
    def test_analyses_cpw_effective_dielectric_constant(self):
        ...
        # Test all elements of the result data against expected data
        for actual, expected in [
            (test_a_actual, 3.5771424204599436),
        ...
        with self.assertRaises(ZeroDivisionError):
            test_a_actual = cpw_calculations.effective_dielectric_constant(
                5 * 10**9, 10 * 10**-6, 0, 760 * 10**-6, 200 * 10**-9, 1.3, 0.2, 1.3
            )

        with self.assertRaises(ZeroDivisionError):
            test_a_actual = cpw_calculations.effective_dielectric_constant(
                5 * 10**9, 10 * 10**-6, 6 * 10**-6, 0, 200 * 10**-9, 1.3, 0.2, 1.3
            )
    ...
    def test_analysis_lumped_ic_from_ej(self):
        ...
        self.assertAlmostEqualRel(
            lumped_capacitive.Ic_from_Ej(5), 1.5198803355538426e16, rel_tol=1e16
        )
    ...
    def test_analyses_lumped_levels_vs_ng_real_units(self):
        ...
        result.append(lumped_capacitive.levels_vs_ng_real_units(0.05005, 0.05005))
        ...
        result.append(lumped_capacitive.levels_vs_ng_real_units(0.05005, 0.05005, N=25))
        result.append(lumped_capacitive.levels_vs_ng_real_units(0.0001, 0.0001, N=25))
    ...
    def test_analyses_lumped_load_q3d_capacitance_matrix(self):
        ...
        data = {
            "ground_plane": [288.290, -32.540, -31.978, -40.063, -43.842, -30.053],
            "Q1_bus_Q0_connector_pad": [
                -32.54000,
                47.25700,
                -0.22765,
        ...
        test_a_df_cmat_expected = pd.DataFrame(
            data,
            index=[
                "ground_plane",
                "Q1_bus_Q0_connector_pad",
        ...
        test_a_df_cond_expected = pd.DataFrame(
            data,
            index=[
                "ground_plane",
                "Q1_bus_Q0_connector_pad",
        ...
        test_b_df_cmat_expected = pd.DataFrame(
            data,
            index=[
                "ground_plane",
                "Q1_bus_Q0_connector_pad",
        ...
        test_b_df_cond_expected = pd.DataFrame(
            data,
            index=[
                "ground_plane",
                "Q1_bus_Q0_connector_pad",
        ...
        self.assertEqual(
            test_a_design_variation_expected, test_a_design_variation_result
        )

        self.assertEqual(4, test_b_return_size_result)
        self.assertEqual("pF", test_b_user_units_result)
        self.assertEqual(
            test_b_design_variation_expected, test_b_design_variation_result
        )
        ...
        for i in range(data_points):
            for j in [
                ...
                self.assertAlmostEqual(
                    test_a_df_cmat_expected[j].iloc[i],
        ...
        for i in range(data_points):
            for j in [
                ...
                self.assertAlmostEqual(
                    test_a_df_cond_expected[j].iloc[i],
        ...
        for i in range(data_points):
            for j in [
                ...
                self.assertAlmostEqual(
                    test_b_df_cmat_expected[j].iloc[i],
        ...
        for i in range(data_points):
            for j in [
                ...
                self.assertAlmostEqual(
                    test_b_df_cond_expected[j].iloc[i],
    ...
    def test_analyses_lumped_df_reorder_matrix_basis(self):
        ...
        for j in [
            ...
            self.assertIterableAlmostEqual(
                test_a_expected[j], test_a_result[j], abs_tol=1e-20
            )
        ...
        for j in [
            ...
            self.assertIterableAlmostEqual(
                test_b_expected[j], test_b_result[j], abs_tol=1e-20
            )
    ...
    def test_analysis_transmon_charge_basis_evaluek(self):
        ...
        self.assertAlmostEqualRel(hcpb.evalue_k(0), -11175.114908531536, rel_tol=1e-6)
        self.assertAlmostEqualRel(hcpb.evalue_k(2), -653.7652579628739, rel_tol=1e-6)
    ...
    def test_analysis_transmon_charge_basis_h0_to_qutip(self):
        ...
        for k in range(1, n):
            self.assertAlmostEqualRel(diag[k], hcpb.fij(0, k), rel_tol=1e-10)
