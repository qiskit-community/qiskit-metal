...
from qiskit_metal.toolbox_metal import (
    about,
...
from .test_data.quad_coupler import QuadCoupler
...
class TestToolboxMetal(unittest.TestCase, AssertionsMixin):
    ...
    def test_toolbox_metal_parse_string_to_float(self):
        ...
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("1um"), 0.001, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("2mm"), 2, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("3m"), 3000.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("4km"), 4000000.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("5cm"), 50.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("6mm"), 6, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("2 mile"), 3218688.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("3 yard"), 2743.2, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing._parse_string_to_float("4 inch"), 101.6, rel_tol=1e-3
        )
    ...
    def test_toolbox_metal_parse_value(self):
        ...
        self.assertAlmostEqualRel(
            parsing.parse_value("1 meter", var_dict), 1000.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("2 um", var_dict), 0.002, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("3 miles", var_dict), 4828032.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("4 feet", var_dict), 1219.1999999999998, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("5 km", var_dict), 5000000.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("6 pm", var_dict), 6.000000000000001e-09, rel_tol=1e-10
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("-12 inches", var_dict),
            -304.79999999999995,
            rel_tol=1e-3,
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("data_a", var_dict), 6437376.0, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("data_b", var_dict), 0.001, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("1 um", None), 0.001, rel_tol=1e-3
        )
        self.assertAlmostEqualRel(
            parsing.parse_value("1 um", "None"), 0.001, rel_tol=1e-3
        )
    ...
    def test_toolbox_metal_parse_options(self):
        ...
        actual.append(
            parsing.parse_options(dict_2, "data_a,data_b,data_c,data_d", dict_1)
        )
    ...
    def test_toolbox_metal_parse_units(self):
        ...
        self.assertEqual(parsing.parse_units([2, 3.5, 5]), [0.002, 0.0035, 0.005])
    ...
    def test_toolbox_metal_get_bounds_of_path_and_poly_tables(self):
        ...
        conn_pads = dict(
            connection_pads=dict(
                coup1=dict(loc_W=-1, loc_H=1), coup2=dict(loc_W=1, loc_H=1)
            )
        )
        q1 = TransmonPocket6(
            multiplanar_design, "Q1", options=dict(**conn_pads, chip="c_chip", layer=1)
        )
        qc1 = QuadCoupler(
            multiplanar_design,
            "qc1",
        ...
        self.assertIsInstance(
            BoundsForPathAndPolyTables(
                multiplanar_design
            ).get_bounds_of_path_and_poly_tables(False, [], 1, 0, 0),
            tuple,
        )
    ...
    def test_toolbox_metal_ensure_component_box_smaller_than_chip_box_(self):
        ...
        self.assertEqual(
            BoundsForPathAndPolyTables(
                multiplanar_design
            ).ensure_component_box_smaller_than_chip_box_(
                (0, 0, 2, 3), (x_0, y_0, x_1, y_1)
            ),
            (0, 0, 2, 3),
        )
        self.assertEqual(
    ...
    def test_toolbox_metal_get_box_for_xy_bounds(self):
        ...
        self.assertEqual(
            BoundsForPathAndPolyTables(multiplanar_design).get_box_for_xy_bounds(),
            (-4.5, -3.5, 4.5, 3.5),
        )
    ...
    def test_toolbox_metal_get_x_y_for_chip(self):
        ...
        self.assertEqual(
            BoundsForPathAndPolyTables(multiplanar_design).get_x_y_for_chip("c_chip"),
            ((-4.5, -3.5, 4.5, 3.5), 0),
        )
        self.assertEqual(
    ...
    def test_toolbox_metal_chip_names_not_in_design(self):
        ...
        self.assertEqual(
            BoundsForPathAndPolyTables(multiplanar_design).chip_names_not_in_design(
                set(multiplanar_design.chips.keys()),
                multiplanar_design.ls.get_unique_chip_names(),
            ),
            None,
        )
    ...
    def test_toolbox_metal_chip_size_not_in_chipname_within_design(self):
        ...
        self.assertEqual(
            BoundsForPathAndPolyTables(
                multiplanar_design
            ).chip_size_not_in_chipname_within_design("c_chip"),
            None,
        )
    ...
    def test_toolbox_metal_get_layer_datatype_when_fill_is_true(self):
        ...
        self.assertEqual(
            LayerStackHandler(
                multiplanar_design, (ls_file_path, None)
            ).get_layer_datatype_when_fill_is_true(),
            {
                (1, 0): {
                    "layer": 1,
                    "datatype": 0,
    ...
    def test_toolbox_metal_get_properties_for_layer_datatype(self):
        ...
        self.assertEqual(
            LayerStackHandler(
                multiplanar_design, (ls_file_path, None)
            ).get_properties_for_layer_datatype(["material", "thickness"], 1, 0),
            ("pec", 0.002),
        )
        self.assertEqual(
            LayerStackHandler(
                multiplanar_design, (ls_file_path, None)
            ).get_properties_for_layer_datatype(["material", "thickness"], 3, 0),
            ("silicon", -0.5),
        )
    ...
    def test_toolbox_metal_read_csv_df(self):
        ...
        self.assertEqual(
            LayerStackHandler(multiplanar_design, (ls_file_path, None))._read_csv_df(
                ls_file_path
            ),
            None,
        )
    ...
    def test_toolbox_metal_warning_search(self):
        ...
        self.assertEqual(
            LayerStackHandler(multiplanar_design, (ls_file_path, None))._warning_search(
                "test_chip", 7, 1, "FAIL"
            ),
            None,
        )
    ...
    def test_toolbox_metal_warning_search_minus_chip(self):
        ...
        self.assertEqual(
            LayerStackHandler(
                multiplanar_design, (ls_file_path, None)
            )._warning_search_minus_chip(5, 1, "FAIL"),
            None,
        )
    ...
    def test_toolbox_metal_layer_stack_handler_pilot_error(self):
        ...
        self.assertEqual(
            LayerStackHandler(
                multiplanar_design, (ls_file_path, None)
            ).layer_stack_handler_pilot_error(),
            None,
        )
