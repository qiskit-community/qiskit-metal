...
class TestSolutionTypeAliases(unittest.TestCase):
    ...
    def test_driventerminal_hfss_2024_aliases_present(self):
        ...
        self.assertIn("HFSS Hybrid Terminal Network", DRIVEN_TERMINAL_NAMES)
    ...
    def test_alias_sets_are_disjoint(self):
        ...
        for i, a in enumerate(all_sets):
            for j, b in enumerate(all_sets):
                if i < j:
                    self.assertEqual(a & b, frozenset(), f"overlap between {a} and {b}")
...
class TestSolutionTypePredicates(unittest.TestCase):
    ...
    def test_predicates_reject_unknown_strings(self):
        for unknown in ("", "eigenmode", "drivenmodal", "HFSSEigenmode", "modal", None):
...
class TestCanonicalKind(unittest.TestCase):
    ...
    def test_drivenmodal_hfss_2024_aliases_map(self):
        self.assertEqual(canonical_kind("HFSS Modal Network"), "drivenmodal")
        self.assertEqual(canonical_kind("HFSS Hybrid Modal Network"), "drivenmodal")

    def test_driventerminal_legacy_maps(self):
        self.assertEqual(canonical_kind("DrivenTerminal"), "driventerminal")

    def test_driventerminal_hfss_2024_aliases_map(self):
        self.assertEqual(canonical_kind("HFSS Terminal Network"), "driventerminal")
        self.assertEqual(
            canonical_kind("HFSS Hybrid Terminal Network"), "driventerminal"
        )
