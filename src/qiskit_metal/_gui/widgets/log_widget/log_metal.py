...
class QTextEditLogger(QTextEdit):
    ...
    def setup_menu(self):
        ...
        ###################
        # Toggle actions
        self.action_scroll_auto = QAction(
            "&Autoscroll", self, checkable=True, checked=True
        )
        self.action_scroll_auto.toggled.connect(self.toggle_autoscroll)

        self.action_show_times = QAction(
            "&Show timestamps", self, checkable=True, checked=False
        )
        ...
        actions.info = QAction(
            "Set filter level:  Info", self, triggered=make_trg(logging.INFO)
        )
    ...
    def set_window_title_level(self, level: int):
        ...
        if self._level_name not in [""]:
            ...
            self.dock_window.setWindowTitle("Log (right click log for options)")
