...
class BridgeFreeJunction(QComponent):
    ...
    def make(self):
        # Pre-existing scaffold variable (kept for parity with the
        # example notebook accompanying this component).
        _name = "bf_JJ"  # noqa: F841
        ...
        wire_l = draw.box(
            -(p.wire_length + p.JJ_width / 2.0),
            -p.wire_width / 2.0 - shift_2_t,
            -p.JJ_width / 2.0,
            p.wire_width / 2.0 - shift_2_b,
        )

        # Define the wire at the right side of the junction.
        wire_r = draw.box(
            p.JJ_width / 2.0,
            -p.wire_width / 2.0 + shift_1_b,
            p.JJ_width / 2.0 + p.wire_length,
            p.wire_width / 2.0 + shift_1_t,
        )

        # Draw the Junction area as a box
        JJ = draw.box(
            -p.JJ_width / 2.0,
            -p.JJ_height / 2.0 - shift_2_t,
            p.JJ_width / 2.0,
            p.JJ_height / 2.0 + shift_1_t,
        )
        ...
        undercut_jj_low = draw.box(
            -p.JJ_width / 2.0,
            -p.JJ_height / 2.0 - shift_2_t,
            p.JJ_width / 2,
            -p.JJ_height / 2.0 - shift_2_t - shift_1_b,
        )

        # Define undercut layer above the junction (for the top electrode)
        undercut_jj_up = draw.box(
            -p.JJ_width / 2.0,
            p.JJ_height / 2.0 + shift_1_t,
            p.JJ_width / 2,
            p.JJ_height / 2.0 + shift_1_t + shift_2_b,
        )
