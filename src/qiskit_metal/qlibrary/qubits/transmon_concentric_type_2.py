...
class TransmonConcentricType2(BaseQubit):
    ...
    def make(self):
        ...
        l = (
            p.rad_outer
            + 0.5 * p.finger_NE_length
            + 0.5 * (0.5 * p.finger_NE_length + p.coupler_NE_gap)
        )
        ...
        coupler_NE_cut = draw.rotate(
            coupler_NE_cut,
            -45,
            origin=(l * cos(45 * 3.14159 / 180), l * sin(45 * 3.14159 / 180)),
        )
        ...
        l_nw = (
            p.rad_outer
            + 0.5 * p.finger_NW_length
            + 0.5 * (0.5 * p.finger_NW_length + p.coupler_NW_gap)
        )
        ...
        l_sw = (
            p.rad_outer
            + 0.5 * p.finger_SW_length
            + 0.5 * (0.5 * p.finger_SW_length + p.coupler_SW_gap)
        )
        ...
        coupler_SW_cut = draw.rotate(
            ...
            135,
            origin=(
                -1.0 * l_sw * cos(45 * 3.14159 / 180),
                -1.0 * l_sw * sin(45 * 3.14159 / 180),
            ),
        )
        coupler_SW = draw.subtract(coupler_SW, coupler_SW_cut)
        # draw the east finger with the rectangular coupling pad
        finger_E = draw.LineString(
            [(-1.0 * p.rad_outer, 0.0), (-1.0 * (p.rad_outer + p.finger_E_length), 0.0)]
        )
        box = draw.rectangle(
            p.box_E_width,
            p.box_E_length,
            -1.0 * (p.rad_outer + p.finger_E_length + 0.5 * p.box_E_width),
            0.0,
        )
        # draw the north finger with the triangular coupling pad
        finger_N = draw.LineString(
            [(0.0, p.rad_inner), (0.0, p.rad_inner + p.finger_N_length)]
        )
        ...
        padtop = draw.translate(padtop, 0.0, -0.05 * p.triangle_height)
        ...
        self.add_qgeometry(
            "junction", geom_jj, layer=p.layer, subtract=False, width=p.jj_w
        )
        self.add_qgeometry(
            "path",
            geom_finger_NE,
        ...
        self.add_qgeometry(
            "path", geom_finger_E, layer=p.layer, subtract=False, width=p.finger_E_width
        )
        self.add_qgeometry("poly", geom_box_E, layer=p.layer, subtract=False)
        self.add_qgeometry(
            "path", geom_finger_N, layer=p.layer, subtract=False, width=p.finger_N_width
        )
        self.add_qgeometry("poly", geom_pad_top, layer=p.layer, subtract=False)
        self.add_qgeometry("poly", geom_coupler_NE, layer=p.layer, subtract=False)
        self.add_qgeometry("poly", geom_coupler_NW, layer=p.layer, subtract=False)
        self.add_qgeometry("poly", geom_coupler_SW, layer=p.layer, subtract=False)
        ...
        qp1b_r = (
            p.rad_outer + p.finger_NE_length + p.coupler_NE_gap + p.coupler_NE_width
        )
        qp1b = (qp1b_r * cos(45 * 3.14159 / 180), qp1b_r * sin(45 * 3.14159 / 180))
        qp1a = qpin_rotate_translate(qp1a)
        qp1b = qpin_rotate_translate(qp1b)
        self.add_pin(
            "pin1", points=np.array([qp1a, qp1b]), width=0.01, input_as_norm=True
        )
        # Northwest coupler
        qp2a_r = p.rad_outer + p.finger_NW_length + p.coupler_NW_gap
        qp2a = (
            -1.0 * qp2a_r * cos(45 * 3.14159 / 180),
            qp2a_r * sin(45 * 3.14159 / 180),
        )
        qp2b_r = (
            p.rad_outer + p.finger_NW_length + p.coupler_NW_gap + p.coupler_NW_width
        )
        qp2b = (
            -1.0 * qp2b_r * cos(45 * 3.14159 / 180),
            qp2b_r * sin(45 * 3.14159 / 180),
        )
        qp2a = qpin_rotate_translate(qp2a)
        qp2b = qpin_rotate_translate(qp2b)
        self.add_pin(
            "pin2", points=np.array([qp2a, qp2b]), width=0.01, input_as_norm=True
        )
        # Southwest coupler
        qp3a_r = p.rad_outer + p.finger_SW_length + p.coupler_SW_gap
        qp3a = (
            -1.0 * qp3a_r * cos(45 * 3.14159 / 180),
            -1.0 * qp3a_r * sin(45 * 3.14159 / 180),
        )
        qp3b_r = (
            p.rad_outer + p.finger_SW_length + p.coupler_SW_gap + p.coupler_SW_width
        )
        qp3b = (
            -1.0 * qp3b_r * cos(45 * 3.14159 / 180),
            -1.0 * qp3b_r * sin(45 * 3.14159 / 180),
        )
        qp3a = qpin_rotate_translate(qp3a)
        qp3b = qpin_rotate_translate(qp3b)
        self.add_pin(
            "pin3", points=np.array([qp3a, qp3b]), width=0.01, input_as_norm=True
        )
