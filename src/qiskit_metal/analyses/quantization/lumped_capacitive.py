...
def extract_transmon_coupled_Noscillator(
    ...
    for ii in range(N):
        for jj in range(N):
            tCqbusbus[ii, jj] = Cbusbus[ii, jj] + (Cbus[0, ii] + Cbus[1, ii]) * (
                Cbus[0, jj] + Cbus[1, jj]
            ) / (C1S + C2S)
    ...
    # d = -EC
    d = alpha * 2 * np.pi * 1e6
    Chi_in_MHz = 2 * chi(gqbus, wr, wq, d + wq) / 2 / np.pi / 1e6  # Total chi in MHz
...
def get_C_and_Ic(Cin_est, Icin_est, f01, f02on2):
    ...
    xrr = opt.minimize(
        lambda x: cos_to_mega_and_delta(x[0], x[1], f01, f02on2),
        [Cin_est, Icin_est],
        tol=1e-4,
...
def cos_to_mega_and_delta(Cin, ICin, f01, f02on2):
    ...
    return (
        (fqubitGHz - f01) ** 2 + (fqubitGHz + anharMHz / 2.0 / 1e3 - f02on2) ** 2
    ) ** 0.5
...
def chargeline_T1(Ccharge, Cq, f01):
    ...
    return Cq / (Ccharge**2 * 50.0 * (2 * np.pi * f01) ** 2)


# TODO: Move to a more generic file


def readin_q3d_matrix(path: str, delim_whitespace: bool = True):
    ...
    if delim_whitespace is True:
        sep = r"\s+"
    else:
        sep = None

    df_cmat = pd.read_csv(
        io.StringIO(s2[0].strip()), sep=sep, skipinitialspace=True, index_col=0
    )
    if len(s2) > 1:
        df_cond = pd.read_csv(
            io.StringIO(s2[1].strip()), sep=sep, skipinitialspace=True, index_col=0
        )
    else:
        df_cond = None

    if delim_whitespace is False and len(df_cmat.columns):
...
def lumped_oscillator_from_path(
    path: str,
    Lj_nH: float,
    Cj_fF: float,
    N: int,
    ...
    RES = extract_transmon_coupled_Noscillator(
        df_cmat.values * c_units, IC_Amps, CJ, N, fb, fr, g_scale=1, print_info=True
    )
